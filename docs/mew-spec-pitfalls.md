...
## 踩坑 42：Agent Team 各模块都“完成”了，组合起来却不是一个可工作的系统

这是 Agent Team 章节最值得保留的一次系统性返工。前面的踩坑 37 到 41 分别记录了配置开关、角色热加载、工具作用域、后台通知和任务闭环的问题；把它们放在一起看，真正的大坑不是某个函数写错，而是把一个分布式工作流误当成若干独立功能点交付。

### 现象

- Team、Task、Mailbox、Worktree、后台成员和 Integration 都有实现，各自的单元测试也能通过。
- 真实模型却可能用普通 `Agent` 代替 Team 成员，或者只发消息不分配任务。
- 成员有 roster、有 Worktree、有任务记录，但可能没有活着的 worker，也可能拿不到写工具。
- 主工作区中的未跟踪文件不会自动进入 Worktree，成员看到的输入与用户看到的输入不同。
- 成员回复“完成”并不代表产生了 Git commit，Lead 也没有等待和汇总结果，最终修改无法回到主工作区。
- TUI 同时接收用户输入、权限确认和后台通知，异步输出会干扰确认状态，进一步放大失败。

最终形成了一个很有迷惑性的状态：控制面显示团队存在、任务存在、成员 idle 或 completed，但数据面没有产生任何可交付修改。系统看起来每一层都在工作，用户目标却没有完成。

### 根因

1. **验收边界错了。** 早期验收分别停在“工具已注册”“任务已入队”“Worktree 已创建”“成员有回复”，没有以主工作区出现正确修改作为唯一完成标准。
2. **控制面和执行面脱节。** TeamTask、TeamMessage、邮箱唤醒、成员状态分别落盘，却没有一个持久化状态机保证任务从 pending 走到 assigned、running、committed、integrated。
3. **可靠性错误地依赖瞬时事件。** 邮箱消息和 wake event 被当作任务事实来源；worker 错过唤醒或进程退出后，roster 仍可能显示正常，也无法从任务表恢复。
4. **跨模块契约没有统一。** 权限系统把非交互成员限制为只读，Worktree 不包含未跟踪输入，Git 集成又拒绝存在无关未跟踪文件；每个模块单独合理，组合后互相封死。
5. **只约束提示词，没有约束能力边界。** Lead 虽然被提示使用 Team，仍能看到普通 Agent；模型选择了更熟悉但语义错误的工具路径。
6. **测试替身过于理想。** Fake Provider 和局部测试没有覆盖真实 Git 仓库、Windows 后台进程、非交互权限、动态工具作用域和同一轮状态变化。

### 重构方案

- 把团队执行定义成一条明确的事务链：`create task -> assign -> claim -> execute -> commit -> wait -> integrate -> publish`。
- 以共享任务表作为事实来源，邮箱和 wake event 只负责通知；worker 重启后可根据持久化 assignee 自领未完成任务。
- Team 激活后在工具作用域中移除普通 Agent，从执行层消除错误路径，而不是继续依赖模型自觉。
- 给任务增加 `paths`，派发前用共享 Git 基线把未跟踪输入安全送进所有成员 Worktree，并记录哈希防止覆盖用户后续修改。
- 非审批成员使用适合无人值守的权限模式，同时继续执行黑名单、路径沙箱、显式 deny 和角色限制。
- 成员自然结束后自动提交；Lead 使用 `wait` 等待终态，再通过 Integration 合并和发布。
- 恢复旧团队时修复 assignee、回收失效 worker、重建输入基线，并在 Windows 上优先使用可可靠追踪的协程后端。
- 把后台输出调度回 TUI 事件循环，完整结果持久化，通知截断只影响展示而不改变任务状态。

### 验证方式

最终没有再用“某个对象创建成功”证明 Team 可用，而是增加真实 Git 端到端场景：主工作区存在一份未跟踪文件，两个成员在独立 Worktree 并行修改不同区块，各自自动提交，Lead 等待全部完成后依次集成，最终主工作区同时包含两人的修改，且无关未跟踪文件保持不变。

这类测试同时覆盖了模型编排之外最容易断裂的契约：输入基线、路径隔离、非交互写权限、成员生命周期、提交产物、分支共同祖先、发布保护和最终用户可见结果。

### 面试讲述版本（STAR）

**Situation：** 我在开发一个类似 Claude Code 的终端 Coding Agent。Agent Team 章节需要让 Lead 派两个长期成员在独立 Worktree 并行工作。初版完成后，Team、任务、邮箱、Worktree 和集成模块的测试都通过了，但真实使用时成员不会稳定领任务，即使手动分配也只会返回只读说明，修改无法回到主分支。

**Task：** 我的目标不是修一个报错，而是定位为什么“模块全部通过”仍无法完成一次真实团队任务，并建立可恢复、可验证的执行闭环。

**Action：** 我先沿着自然语言请求追踪整条链路，发现控制面状态与实际执行脱节：消息不等于任务分配、唤醒不等于可靠队列、Worktree 不包含未跟踪输入、非交互权限默认只读、成员回复不等于 Git 产物。随后我把共享任务表改成事实来源，补齐 assign、claim、commit、wait 和 integrate 状态链；用工具作用域硬隔离普通 Agent；增加共享 Git 基线解决未跟踪文件；让成员自动提交，并为旧任务恢复 assignee 和 worker。最后用两个真实并行成员修改同一文件不同区块的 Git 端到端测试验收。

**Result：** 修复后，两个成员能够在独立 Worktree 并行修改、自动提交并集成回主工作区；Team 专项 30 项、全量 410 项测试通过。更重要的是，验收标准从“组件已创建”改成“用户目标已在主工作区正确落地”，之后的并发 Agent 功能都复用这条原则。

### 面试时值得强调的判断

- 分布式或异步系统不能把 UI 状态、通知消息和进程存活当作业务完成事实。
- 持久化状态负责可靠性，事件负责降低延迟；两者职责不能颠倒。
- 多模块系统最危险的不是模块内部错误，而是局部正确的策略组合后互相冲突。
- Agent 的提示词只是软约束，关键流程必须通过工具作用域、状态机和事务边界做硬约束。
- 端到端测试应验证最终业务结果以及关键副作用，而不是只验证中间对象和返回值。

以后写 spec 要补：
- 为跨模块能力先画出唯一的端到端状态链，并为每个状态定义事实来源、恢复方式和终态产物。
- Checklist 最后一项必须从用户真实入口启动，使用真实文件系统和版本库，验证最终结果落到用户可见位置。
- 每个后台执行单元都要回答：错过通知怎么办、进程退出怎么办、重复消费怎么办、如何证明结果已提交。
- 对权限、沙箱、Worktree、缓存和集成等横切模块做组合测试，不能用各模块单测通过代替契约验证。
