...
- 全量测试：`python -m unittest discover -v`
  - 结果：286 项通过，2 项跳过，0 失败（含真实使用后的补充回归）。
...
| AC18 | 通过 | 286 项测试、compileall、diff check 和 README 对照通过；tmux 受环境限制，使用 CLI fake-provider 场景替代。 |
...
- 新增 `/skill [name]` 本地命令；真实 Python 3.11 下 `/skill` 列出 4 个 Skill，`/help` 显示 `/frontend-design [arguments]`。
- PyYAML 缺失从“逐文件 warning 并继续空 Catalog”提升为启动级 `SkillConfigError`，错误直接给出安装命令。
