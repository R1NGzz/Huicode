...
- 真实闭环验证：主分支存在未跟踪 `DEMO.md` 和无关未跟踪文件，两个 coroutine 成员从共享基线并行修改不同 section、自动提交，Lead 等待终态后合并并 fast-forward 发布；最终文件同时包含两人修改，无关文件保持原样。
...
| AC13 回归验证 | 通过 | 410 项全量测试通过，旧配置默认关闭 Team 能力，实际交付配置显式开启 |
