...
class CLIPlanModeTests(unittest.TestCase):
    ...
    def test_plan_then_do_injects_recent_plan(self) -> None:
        provider = InspectableProvider()
        output = io.StringIO()
        with patch("builtins.input", side_effect=["/plan 先做计划", "/do", "/exit"]), redirect_stdout(output):
            exit_code = _run_chat(provider, self.config)
    ...
    def test_clear_resets_recent_plan(self) -> None:
        provider = InspectableProvider()
        output = io.StringIO()
        with patch("builtins.input", side_effect=["/plan 先计划", "/clear", "/do", "/exit"]), redirect_stdout(output):
            exit_code = _run_chat(provider, self.config)

        self.assertEqual(exit_code, 0)
        self.assertEqual(len(provider.calls), 1)
        self.assertIn("当前还没有最近计划", output.getvalue())
